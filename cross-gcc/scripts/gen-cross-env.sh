...
EOF
	    ...
	    if [ "x${GENERATE_LUA_MODULE}" != "x" ]; then

		# Lua version
		cat <<EOF > "${MODULE_ENVIRONMENT_DIR}/${mod_file}.lua"
--
--  ${cpu} gcc toolchain for ${type} binary
--
--  Note: This is generated by gen-cross-env.sh
...
help (
[[
${cpu} gcc toolchain for ${type} binary Setting \n
]])

whatis("${cpu} gcc toolchain for ${type} binary Setting")
...
-- append pathes
prepend_path("PATH",os.getenv("HOME").."/${cross_dir}/bin")
prepend_path("LD_LIBRARY_PATH",os.getenv("HOME").."/${cross_dir}/lib64")
prepend_path("LD_LIBRARY_PATH",os.getenv("HOME").."/${cross_dir}/lib")
